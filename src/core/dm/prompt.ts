...
export function formatDmUserMessage(input: DmTurnInput): string {
  ...
  const actDiscipline =
    world?.actObjective === 'meet-counterexample'
      ? '本幕是最后一幕，两件事同时做到：① 必须让玩家遇见一个与前两幕不同的经验逻辑；② 不再引入新的现实主张，只收敛「这一局看清了什么、还剩哪个未知必须由现实回答」。反例出现的那一刻，用刘看山的口吻说一句：「等等。这个人的结果和前面完全相反。」（这是刘看山全篇第二次出现，也是最后一次在本幕出现）。如果【允许引用的真实经验】里没有任何反例，不得虚构反例、不得拿支持性内容冒充 —— 如实呈现「我们没找到反例」这件事本身。'
      : null;
