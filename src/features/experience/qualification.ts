...
function assignedTrackOf(
  ...
  if (purposes.includes('similar-person') && matchedConstraints.length > 0) return 'similar';
  return 'adjacent';
}
...
export function qualifyExperienceSource(input: {
  ...
  const targetHits = targetTerms.filter((term) => sourceText.toLowerCase().includes(term)).length;
  const topicScore = targetTerms.length > 0 ? targetHits / targetTerms.length : 0;
  const topicRelation = topicScore >= 0.22 ? 'relevant' : topicScore >= 0.08 ? 'partial' : 'irrelevant';
  ...
  const eligibleAsCase =
    topicRelation !== 'irrelevant' &&
    firsthand === 'yes' &&
    commercialRisk !== 'high' &&
    (completeness.action || completeness.outcome);
  const assignedTrack = assignedTrackOf(input.purposes, matchedConstraints);
  ...
  if (assignedTrack === 'adjacent') reasons.push('目标相关，但没有命中用户明确条件');
