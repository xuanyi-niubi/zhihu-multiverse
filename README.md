...
| 位置 | 文件 | 说明 |
|---|---|---|
| 首页 | `public/screenshots/home.png` | 命运观象厅 + 唯一 CTA |
| World Forge | `public/screenshots/world-forge.png` | 三条轨道 + 真实片段 materialize |
| Hidden Path Reveal | `public/screenshots/path-reveal.png` | 一条原本不存在的轨道亮起来 |
| Endgame | `public/screenshots/endgame.png` | 问题重写 + 现实支线 |

> 文件尚未入库前**不嵌入图片**（避免 README 出现坏图）；落地后把上表中间一列替换成
> `![首页](./public/screenshots/home.png)` 即可。
...
| **Counterexample** —— 第三幕必须由真实反例驱动，找不到就诚实留空 | 「原来看起来对的路，有人是这样走坏的」 | `features/game-world/compileWorld.ts` |
| **Unknown Lock** —— 只有现实能回答的事，在游戏里永远锁着 | 「这个不是 AI 能替我猜的」 | `features/game-mechanics/unknownLock.ts` |
| **Reality Pass** —— 终局不判卷，还给玩家一条带成功信号与停止信号的现实支线 | 「我知道接下来该验证什么了」 | `features/game-world/questView.ts` |

**唯一的核心命题**：别人真实走过的人生，会改变玩家现在能够看见的选择。
...
ProblemFrame
  ↓
Zhihu Experience Retrieval
  ↓
Experience Facts / Cases
  ↓
World Blueprint
  ↓
Action Space
...
每一格都对应一个可以单独测试的纯函数模块，下面是同一张图的可读版本
...
| 玩家看到 | 叙事 + 选项 + 借来的经验 + 终局问题重写 | 数值属性、骰子、遗物、判卷等旧机制（见文末 **Legacy** 一节） |

**旧机制已退出主路径**（只隐藏、不删代码）：数值属性、骰子检定、遗物面板、命途树、
证据网格 UI、现实锚点、清晰度雷达、Boss 判卷、赛博契约、黄金 Case 入口。
它们的代码仍在仓库里服务于 legacy 路径与降级，但玩家在新主链上一次都见不到 ——
相关机制统一登记在文末的 **Legacy：旧 RPG 系统（非当前主链）**。
...
```bash
cp .env.production.example .env.production   # 只需填 A 段：APP_LLM_API_KEY / APP_ZHIHU_ACCESS_SECRET
docker compose up -d --build                 # 首次约 2–4 分钟
```

**公开比赛版的最小配置就是 A 段**：配好 App 能力后，评委打开站点**不需要任何 Key**
（`/settings` 第一屏会写「当前使用：官方体验配置」）。完全留空也能演示 ——
自动走离线剧本 + 已落盘的真实来源快照，只是内容不会现场检索。

**App provider 与访客的关系（§15）**：

- App provider 只是**部署者自己的 server env**（`APP_LLM_*` / `APP_ZHIHU_ACCESS_SECRET`），
  只在服务端读取，**不会发送给浏览器**；
- 访客仍可用 **BYOK 覆盖默认模型**：在 `/settings` 填自己的 Key 后花自己的额度，
  不受 `APP_MAX_*` 的用量闸门限制；
- 用量闸门只对 App 能力生效，用满只把这一回合降级成离线叙事，**不会把站点打成 5xx**。
...
分三段：**Public App Runtime**（部署者填一次，所有访客打开即用）、
**知乎账号登录**（可选）、**Advanced / legacy**（主链已不读取的历史配置）。
逐项注释与完整清单以 [`.env.example`](.env.example) / [`.env.production.example`](.env.production.example) 为准。

#### A. Public App Runtime（公开比赛版只需要这一段）

| 变量 | 默认 | 说明 |
|---|---|---|
| `APP_LLM_API_KEY` | 空 | 服务器自己的模型密钥；配了它，访客**不需要填任何 Key** |
| `APP_LLM_BASE_URL` | `https://api.deepseek.com/v1` | 任意 OpenAI 兼容端点 |
| `APP_LLM_FAST_MODEL` | 空（跟随 DEEP） | 便宜档：理解与单幕叙事 |
| `APP_LLM_DEEP_MODEL` | 空（跟随 FAST） | 能力档：世界编译 |
| `APP_LLM_JSON_MODE` | `true` | 要求 `response_format: json_object`；端点不支持时才关 |
| `APP_ZHIHU_ACCESS_SECRET` | 空 | 服务器自己的知乎 Access Secret（真实来源检索） |
| `APP_MAX_LLM_CALLS_PER_SESSION` | `5` | 单局最多几次 App 模型调用；超了只降级离线叙事 |
| `APP_MAX_SESSIONS_PER_HOUR` | `5` | 每个身份每小时最多开几局（按匿名 cookie / 账号，不用 IP） |
| `APP_MAX_SESSIONS_PER_DAY` | `20` | 每个身份每天最多开几局 |
| `APP_MAX_DAILY_LLM_CALLS` | `2000` | 全站每天 App 调用软上限；用满只暂停 App 能力，BYOK 照常 |

#### B. 知乎账号登录（可选）

| 变量 | 说明 |
|---|---|
| `ZHIHU_OAUTH_APP_ID` | App ID（短数字），来自开放平台应用详情页 |
| `ZHIHU_OAUTH_APP_KEY` | OAuth App Key，用于 `/access_token` 换取用户令牌 |
| `ZHIHU_OAUTH_REDIRECT_URI` | 公网 HTTPS 回调，须与开放平台登记值完全一致 |

#### C. Advanced / legacy

| 变量 | 说明 |
|---|---|
| `DM_MODEL` / `DM_FAST_MODEL` / `DM_DEEP_MODEL` | 档位名覆盖（生效）：优先于 `APP_LLM_*_MODEL` |
| `MEMORY_DIR` | 账号记忆落盘目录，默认 `./data/memory`；容器里指向挂载卷 |
| `DM_DEBUG_DELAY_MS` | 调试用：给 `/api/dm` 注入人工延迟，生产保持为空 |
| `DM_API_KEY` / `DM_BASE_URL` / `DM_JSON_MODE` / `DM_TIMEOUT_MS` / `DM_TEMPERATURE` / `DM_MAX_TOKENS` / `OPENAI_API_KEY` | **主链已不读取**：仅 legacy 解析器与旧测试使用，新部署不要依赖 |
| `ZHIHU_API_KEY` / `ZHIHU_ACCESS_TOKEN` / `ZHIHU_BASE_URL` / `ZHIHU_CACHE_TTL_MS` | 同上（legacy）。`ZHIHU_ACCESS_SECRET` 只剩 OAuth 凭证诊断与 legacy 解析器读取 |

**三件事必须说清（05_AGENT §15）**：

1. **App provider 是部署者自己的 server env**：`APP_LLM_*` / `APP_ZHIHU_ACCESS_SECRET`
   只在服务端读取（`src/config/serverEnv.ts` 有运行期闸门），**绝不发送给浏览器** ——
   页面源码、Network 响应、localStorage 里都搜不到密钥值；
2. **BYOK 仍可覆盖默认模型**：访客在应用内 `/settings` 填自己的 Key 后，
   解析优先级变成 `用户自带 > App > none`，花的是他自己的额度，也不受上面那些 `APP_MAX_*` 限制；
3. **都不配置也能跑**：自动走离线剧本 + 已落盘的真实来源快照，演示不白屏。
   未登录则记忆功能自动隐藏 —— 游客可完整体验完整一局，只是这局不会被记住。

### 知乎开放平台接入

配置 `APP_ZHIHU_ACCESS_SECRET`（自托管时也可让访客在 `/settings` 填自己的）后，检索链路升级为：

```
玩家目标 → 知乎搜索 API（过滤权威度 ≥ 2）→ 真实语料注入 Prompt
        → 逐字校验 exactQuote → 世界蓝图编译
...
> 图中 ④ 的「属性 / 遗物」校验项属于 **Legacy 推演引擎**；新主链只走关卡结构与逐字引用校验
> （`exactQuote` 必须是原文子串），不发放遗物、不结算属性。
...
APP_ZHIHU_ACCESS_SECRET=<你的 secret> npm run sync:zhihu
```
...
## Legacy：旧 RPG 系统（非当前主链）

> ⚠️ **本节机制已退出主链**，只为兼容旧推演路径（`/play?goal=`、旧挑战链接、
> `/compare`、`/commitment`）与降级路径而保留。新主链
> （`/session/[id]` → `/play?session=<id>`）不使用骰面、DC、属性、遗物、Boss 判卷与 SAN。
> 保留历史设计是为了不让文档引用断裂，**不代表它们是当前玩法**。
>
> 新主链的规则只有四条：**经验就是关卡 / 差异就是冲突 / 行动解锁就是成长 / 把问题问清楚就是通关。**
...
这一页不是「密钥配置页」，而是**设置**。普通玩家打开看到的第一件事是「不需要配置」：

```text
当前使用：官方体验配置      ← 服务器已提供默认能力（只在 /api/health 取"来源结论"，不展示 Key / 端点）
体验    ：跟随系统 / 减少动画 / 不需要额外降级
高级 · 自带模型            ← 默认折叠：API Key / Base URL / Model / JSON mode
  └ 自托管 / 开发设置      ← 再折一层：知乎开放平台 Access Secret（普通玩家不需要）
```

**普通用户不需要任何 Key**：默认能力来自部署者的 `APP_*`（见上文环境变量）。
BYOK（Bring Your Own Key）是**可选覆盖项**，不是使用前提：

> 你也可以使用自己的兼容模型配置。这不会影响默认体验。

「减少动画」是真正生效的偏好（`features/run/appearance.ts` → `<html data-reduce-motion="on">`）：
系统 `prefers-reduced-motion` 永远优先，用户偏好只是额外多降一档。
**文本速度**目前不做开关 —— 正文用的是句读式呈现而不是打字机，放一个点了没反应的控件不如不放。

四条安全承诺（都有自动化测试盯着）：

1. **接口永不返回明文** —— GET 只给「是否配置 + 长度 + sha256 前 8 位」，改 key 就是整把替换；
2. **明文只落服务器磁盘**（`data/settings/*.json`，权限 0600），且不写日志（失败只记错误码）；
3. **按身份隔离** —— 登录按知乎 `url_token`、未登录按匿名 cookie，两种身份互不可见；
4. **删除是真删** —— 单字段删除后文件里就没有它了；删掉最后一项连文件一起移除，不留空壳。

运行时优先级恒为 **用户自带配置 > App provider（`APP_*`）> none（离线兜底）**：
`keyResolution.ts` 统一解析，`/api/dm`、`/api/sessions`、`/api/profile`、`/api/zhihu/hot`
都走它，所以「填了 key 却还在用别人的」这种情况不会发生（有测试专门守它）。

---

## 确定性设计（Legacy 推演引擎）

> ⚠️ 本节只描述 **Legacy 推演引擎**（骰面 / 属性 / 遗物结算 / 种子挑战）。
> 新主链（`/session/[id]`）不掷骰、不算属性、不发遗物：它的确定性来自纯函数编译
> （同一份经验事实必然编译出同一份世界蓝图，见「测试覆盖报告」里的 `worldBlueprint` 系列用例）。
...
```
90 个测试文件 · 1426 个用例 · 通过 1426 · 失败 0
```

> 迁移期曾有 4 条失败，全部来自**旧设计契约**（`tests/observatory.test.ts`
> 仍按已删除的 `--obs-*` / `.obs-*` 断言）。这些契约已随设计系统迁移
> 同步改写 —— 不是把断言放宽，而是把**读取目标**改到 `silver.css`
> 并把选择器换成现名，同时补回了两处真实回归：观象厅的
> reduced-motion 降级块、以及 `.sil-fragment--zhihu` 修饰符。

> 表内标 **`[legacy]`** 的条目覆盖旧 RPG 系统（骰面 / 属性 / 遗物 / Boss 判卷 / SAN）。
> 保留它们是为了让降级路径与历史契约不腐化，**新主链不依赖这些机制**。
...
| `tests/dmValidate.test.ts` | **[legacy]** 越界 DC / 属性、缺字段、非法遗物、非 https 链接的钳制与修复 |
| `tests/memoryApi.test.ts` | `/api/memory` 集成：未登录为正常态、写入读回、**同名不同 token 不串数据**、PATCH 封存遗言幂等 |
| `tests/narrative.test.ts` | **[legacy]** SAN 阈值过滤、伏笔占位符替换、同槽位立绘替换、表情随 mood 变化 |
| `tests/origins.test.ts` | **[legacy]** 三流派策略差异、易伤倍率作用域、结算顺序锁定 |
| `tests/dmGenerate.test.ts` | 合法 / 损坏 / 垃圾 / 两轮失败 / 客户端抛异常 / 兜底函数抛异常 |
| `tests/dmEcho.test.ts` | 氛围句**不得出现数字或百分比**（编造统计一律替换），含预置剧本数据层回归 |
| `tests/challenge.test.ts` | **[legacy]** 挑战链接**必须带 scenario**：少了它被挑战者会静默回落到另一场剧本 |
| `tests/runContracts.test.ts` | **[legacy]** Run/Challenge/Boss 契约：Manifest 篡改必须拒绝、AI 修正永远出不了 -3..+5、来源白名单 |
| `tests/runDeterminism.test.ts` | **[legacy]** **P1 退出门槛**：同一 Manifest 重放 100 次，事件计划/骰面/状态逐字节一致；改文案不改计划，改权重改计划 |
| `tests/scenarioCompiler.test.ts` | **[legacy]** 规则筛合法候选、种子定优先级；头名不合法顺延而不重掷；幽灵线不泄露结果 |
| `tests/worldState.test.ts` | **[legacy]** 属性/隐藏状态钳制、NaN 不污染状态、五个隐藏状态的**极性**与定性信号 |
| `tests/scenarioAdapter.test.ts` | **[legacy]** 剧本结果 → 隐藏状态的确定性规则；属性有唯一事实源（不双写） |
| `tests/signalText.test.ts` | **[legacy]** 信号文案五键×四档全覆盖且**不含任何数字**；警报判定只有一处 |
| `tests/knowledgeSource.test.ts` | **只有 verified 来源能出现赞同数**；`scripted` 文案不含数字；数据层禁止手写 `upvotes` |
| `tests/bossJudge.test.ts` | **[legacy]** 终端 Boss：在线 / 超时 / 损坏 JSON / HTTP 失败四条链路都能出终局；修正钳到 -3..+5；天然 1 必败；幂等 |
| `tests/bossTerminal.test.ts` | **[legacy]** 终局分解必须摊开四维/属性/遗物/思路/骰面 vs DC 并标明判卷来源；不泄露隐藏状态 |
| `tests/realityChecklist.test.ts` | **[legacy]** 破壁清单：祈使句+时间盒+可验证信号；**scripted 来源绝不能变成 verified 条目**；限量去重且同局可复现 |
...
| `tests/errorCopy.test.ts` | 错误态文案：三句固定文案一字不改、**技术细节（429 / quota / provider exception / 堆栈）永不出现在页面上** |
| `tests/appearancePreference.test.ts` | 「减少动画」偏好：坏存储不抛、`off` 不等于强制播放动画、只有主动 `on` 才写 `<html data-reduce-motion>` |
| `tests/keyResolution.test.ts` | 运行时优先级：**账号配置压过环境变量**、未登录只吃 env、来源三态标记正确 |
| `tests/agentsEngine.test.ts` | **[legacy]** AI 叙事引擎：动态难度可解释有界、**模型写骰面/DC/胜负会被结构性丢弃**、主备降级、MAG 记忆注入、选项语义叠加代价 |
| `tests/turnComposer.test.ts` | **[legacy]** 结构接线：AI 定结构而数值由合成器算出；**无 key 时回合一字不改**；模型失败不用短模板覆盖正文 |
| `tests/contentScale.test.ts` | **[legacy]** 内容规模被测试钉住：遗物 ≥15 件且**每件都有叙事钩子**、三种效果类型齐全、场景骨架 12 个覆盖全部幕次 |
| `tests/actEngine.test.ts` | **[legacy]** 动态幕系统**契约**：张力预算驱动幕数（各出身预算 88 / 76 / 84，实测幕数 **7–8 幕**，不再硬编码 4）、终局条件不再固定第 4 幕、难度带由状态而非回合号决定。⚠️ 动态幕目前只在**AI 自由推演**路径生效，预置剧本仍是人工精调的 **4 幕**；详见 `DESIGN.md` §0.5 |
| `tests/worldModel.test.ts` | **[legacy]** 世界模型：压力阈值真改 DC、清晰度低看不到 DC、**Boss 判卷影响现实锚点**、锚点<30% 出异常、线索随幕收束、出口无数字 |
...
| `tests/d20.test.ts` | **[legacy]** 天然 1/20 优先级、修正钳制、同种子确定性、无副作用 |
...
头型、发型、五官、服装全部由路径绘制，5 种造型 × 7 种表情由「眉 / 眼 / 嘴」三组独立组合驱动。表情可随剧情与情绪实时切换，无需为每种情绪各出一张图。原创角色全部使用这套 SVG，不依赖任何外部图片素材。

**命运观象厅视觉语言** — `src/components/visual/` · `src/app/globals.css`
这一版不是「换个配色」：Token 冻结了颜色语义（知乎蓝＝真实来源、青蓝＝新路径、琥珀＝反例、紫银＝终局），动效总表只有七个 keyframes，首页与编译页的固定文案由 `tests/observatory.test.ts` 钉住。`prefers-reduced-motion` 与设置页的「减少动画」都能把它降级成静态形态（内容照常出现，只是不再生长）。
...
`NarrativeBeat` 支持场景切换、角色登场退场、阈值叙述、全屏演出。AI DM 只返回一段文本时自动退化为单个旁白节拍，**离线剧本与动态关卡共用同一套演出管线**。（阈值叙述源自 Legacy 推演引擎的旧状态，新主链只用它的舞台调度能力。）
...
**种子挑战裂变（Legacy）** — `app/challenge/page.tsx`
...
> 标 `[legacy]` 的条目服务于旧 RPG 系统与降级路径（代码保留、测试仍在跑）。
> 新主链只经过 `features/experience`、`features/game-world`、`features/game-mechanics`、
> `features/reality-memory`、`components/visual`、`components/game/session`。
...
│   ├── game-mechanics/       # Action Space Lite：行动状态 / 代价闸门 / 经验碰撞 / 未知锁
│   │   └── ...               # 解锁、锁定、移除都带真实依据，不打分、不猜答案
│   └── run/                  # 证据契约、知识来源、现实清单等运行层契约；外观偏好与错误文案
│                             #   ├─ appearance.ts  # 「减少动画」偏好（三态，坏存储不抛）
│                             #   └─ errorCopy.ts   # 错误码 → 人话（§9：技术细节永不进页面）
├── app/
│   ├── page.tsx              # 命运观象厅首页（唯一 CTA；次级入口只有「我的经历 / 关于」）
│   ├── about/page.tsx        # 关于：为什么必须是知乎 + 我们刻意不做什么
│   ├── play/page.tsx         # ★ 推演舱（Session 主链 + Legacy 推演共用入口）
│   ├── session/[id]/page.tsx # 世界生成中转站：澄清 → prepare-world → World Forge → 进入世界
│   ├── journal/page.tsx      # 我的经历（我在纠结什么、看见了哪些真实经历、现实告诉我什么）
│   ├── compare/page.tsx      # [legacy] 双牌对比：同一份证据、两组条件
│   ├── archive/page.tsx      # [legacy] 推演档案
│   ├── commitment/page.tsx   # [legacy] 赛博契约（七天回执）
│   ├── challenge/page.tsx    # [legacy] 种子挑战落地页
│   ├── oauth/page.tsx        # 知乎账号接入面板
│   ├── settings/page.tsx     # 设置：当前使用 / 体验（减少动画）/ 高级 · 自带模型 / 自托管
│   ├── auth/callback/route.ts # OAuth 回调
│   └── api/
│       ├── dm/route.ts       # AI DM 接口（永远 200 + 合法关卡）
│       ├── mesh/route.ts     # 证据网格（含黄金案例快照反查）
│       ├── sessions/route.ts # 建会话 / 列会话（App 能力在这里扣配额）
│       ├── sessions/[id]/route.ts # 读会话 / 推进流程（含 prepare-world 编译世界蓝图）/ 删除
│       ├── boss/evaluate/route.ts # [legacy] 终局判卷
│       ├── commitment/route.ts    # [legacy] 承诺与回执
│       ├── profile/ · report/ · health/ · memory/ · settings/
│       ├── zhihu/hot/route.ts     # 知乎热榜代理
│       └── oauth/            # status / session / start / run-all / logout
├── components/
│   ├── visual/               # 命运观象厅：AstralDial / OrbitField / WorldForge / HiddenPathReveal
│   │                         #   / CounterOrbit / SentenceReforge / RealityPass / ReducedMotion
│   ├── game/session/         # Session 主链推演屏（幕 / 场景 / 选项 / 借来的经验，不碰 Legacy UI）
│   ├── AppearanceBootstrap.tsx # 把「减少动画」偏好落到 <html data-reduce-motion>
│   ├── characters/           # 参数化 SVG 立绘 + 表情库（含刘看山）
│   ├── scenes/               # 分层场景舞台
│   ├── effects/              # 受击飘字 / 幕转场 / 神经信号加载
│   ├── worldline/            # [legacy] 世界线统一组件 + 临界翻转演出
│   ├── archive/              # [legacy] 档案形态的证据记录
│   ├── DialogueBox.tsx       # [legacy] 打字机对话框
│   ├── GameHud.tsx           # [legacy] HUD（血条 + 分段幕次进度）
│   ├── DiceModal.tsx         # [legacy] 命运掷骰 / 现实裁决演出
│   ├── FateTree.tsx          # [legacy] SVG 命运树
│   ├── InventoryBar.tsx      # [legacy] 三槽位遗物栏
...
│   ├── run/                  # [legacy] 运行模式 / 幕次引擎 / 命运事件牌组
│   │                         #   runtimeMode.ts 里的运行模式与引擎指示灯仍被 /api/health 与首页使用
│   ├── d20.ts                # [legacy] 检定引擎
│   ├── relics.ts             # [legacy] 遗物与属性结算
│   ├── usage/                # App 能力用量闸门（每局调用 / 每身份局数 / 全站每日）
│   ├── narrative.ts          # 叙事编排
│   ├── memory.ts             # 跨周期记忆 / 决策画像 / 前世遗念
│   ├── settingsStore.ts      # 应用内凭据存储（只回指纹、永不回明文）
│   ├── oauth/zhihu.ts        # 知乎 OAuth 协议核心
│   ├── zhihu/                # 搜索 / 热榜客户端
│   └── dm/                   # AI DM 五层防御
├── config/
│   └── serverEnv.ts          # APP_* 的唯读入口（浏览器里调用会直接抛错）
├── data/
│   ├── demoCases.ts          # [legacy] 黄金案例（含口语化匹配短语）
│   ├── zhihuSources.generated.json # 真实抓取的来源快照
│   ├── prebuiltScenarios.ts  # [legacy] 预置剧本（4 幕）+ 遗物库
│   ├── origins.ts            # [legacy] 出身流派
│   ├── characters.ts         # 角色库
│   └── scenes.ts             # 场景库
...
```
├── DESIGN.md                 # 唯一设计文档：产品定义 + 游戏机制 + 技术架构 + 视觉规范（旧 RPG 章节已标 Legacy）
├── ZhihuRelic-D20-Spec.ts    # [legacy] D20 与遗物的纯契约规范（12 条不变量）
├── 产品说明计划书.md          # 参赛必交材料
...
├── .env.example              # A 段 App Runtime / B 段 OAuth / C 段 legacy（与代码一致）
├── .env.production.example   # 生产：通常只填 A 段
├── docs/DEMO-SCRIPT.md       # 30 秒口播 + 演示动线 + 展示用词纪律
├── docs/AGENT-05-交付记录.md  # 产品收口（Settings / 文案 / README / 部署说明）交付与验证记录
└── tests/
```
...
30 秒口播、60 秒展开版、演示动线与「可以说什么 / 不能说什么」的用词纪律，
统一放在 [`docs/DEMO-SCRIPT.md`](docs/DEMO-SCRIPT.md)。

---

## 已知限制

- **AI DM 单关耗时受输出长度主导**：一个完整关卡 JSON 约 600 字，生成时间随模型吞吐线性增长。3 分钟演示建议提前跑通一遍；自由推演用神经信号加载动效承接等待。
- 模型偶尔会编造遗物 id（实测 `relic-code-bible`），校验层会静默丢弃该掉落并保住整个关卡——这是防御正常工作的表现（该机制属 Legacy 推演引擎，新主链不再发放遗物）。
- **Legacy 推演引擎**：遗物「套装共鸣」、双骰检定、知识图谱复盘、宿命分歧点等机制尚未实现
  （它们属于旧 RPG 系统，新主链不使用骰子与遗物）。
