...
> 用 3 分钟的平行推演，跨越现实中 3 年的人生迷茫。

把知乎 15 年的真实问答转化为带数值博弈、属性检定与卡牌遗物的**文字 AVG 推演机**。玩家在**多幕剧**里「试穿」不同的人生结局，每一句知乎金句都可溯源到站内原回答。
...
默认路径只有一条（2026-09-13 收口）：**首页一句话 → 决策会话 → 编译世界 → 进入推演**。

| | 默认主链 | 「跳过澄清，直接推演」 | 「只算账，不推演」 |
|---|---|---|---|
| 入口 | 首页「生成我的平行宇宙」→ `/session/[id]` → `/play?session=<id>` | 首页次级按钮 → `/play?goal=` | 首页次级链接 → `/compare` |
| 做什么 | 动态澄清 → 多意图检索真实经历 → 四幕世界蓝图 → 经验解锁 → 现实支线 | 直接进 AI 自由推演（不做条件澄清，也不编译真实经历） | 只把真实走法摆出来对照，不含游戏 |
| 谁在判断 | 不做裁决；结论交给你七天后要做的那个实验 | 规则引擎裁决，骰子只决定遭遇 | **不做裁决**，只做对照 |
| 定位 | 产品本体 | legacy 兜底 / 开发调试 | 旁路 |
...
输入真实困惑（POST /api/sessions）
  ...
  → 终局把「任何人替不了的那个问题」交还给你，并给一条有停止信号的现实支线
  → 七天后的回访结果被记成**观测到的**记忆（不是自我估计），下一次提问时当硬条件用
```
